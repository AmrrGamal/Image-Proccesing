...
def low_pass_filter(image):
    ...
    filtered_array = cv2.filter2D(img_array, -1, mask_3x3_low_pass)
    filtered_array = np.clip(filtered_array, 0, 255)
    
    return Image.fromarray(filtered_array.astype(np.uint8))


def display_low_pass_filter_result(master, original_image):
   if original_image:
        window = tk.Toplevel(master)
        window.title("Low Pass Filter Result")
        ...
        image = low_pass_filter(original_image)

        if image:
            original_resized = ImageTk.PhotoImage(original_image.resize((400, 300)))
            resized = ImageTk.PhotoImage(image.resize((400, 300)))

            main_title = tk.Label(window, text="Low Pass Filter Result", font=("Arial", 18, "bold"), fg="#4a90e2", bg="#f0f8ff")
            ...
            text = tk.Label(window, text="Low Pass Filter Image", font=("Arial", 14, "bold"), fg="#4a90e2", bg="#f0f8ff")
