...
def display_variance_result(master, original_image):
   if original_image:
        window = tk.Toplevel(master)
        window.title("Variance Result")
        ...
        image = variance_operator(original_image)

        if image:
            original_resized = ImageTk.PhotoImage(original_image.resize((400, 300)))
            resized = ImageTk.PhotoImage(image.resize((400, 300)))

            main_title = tk.Label(window, text="Variance Result", font=("Arial", 18, "bold"), fg="#4a90e2", bg="#f0f8ff")
            ...
            text = tk.Label(window, text="Variance Image", font=("Arial", 14, "bold"), fg="#4a90e2", bg="#f0f8ff")
