...
def display_difference_result(master, original_image ,threshold):
   if original_image:
        ...
        image = difference_operator(original_image,threshold)

        if image:
            original_resized = ImageTk.PhotoImage(original_image.resize((400, 300)))
            resized = ImageTk.PhotoImage(image.resize((400, 300)))

            main_title = tk.Label(window, text="Difference Result", font=("Arial", 18, "bold"), fg="#4a90e2", bg="#f0f8ff")
